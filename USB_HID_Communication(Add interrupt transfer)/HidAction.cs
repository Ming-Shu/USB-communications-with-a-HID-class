...
namespace USB_HID_Communication
{
    public partial class Form1  
    {
		...
		///  Assumes report ID = 0.
		///  </summary>
        private void SendOutputReport(Byte[] outputBuffer,int size,String selectType)
        {
            try
            {
                ...
                else
                {
                    ...
                    else//Don't attempt to exchange reports if valid handles aren't available (as for a mouse or keyboard.)
                    {
                        ...
                        {
                            var outputReportBuffer = new Byte[_myHid.Capabilities.OutputReportByteLength];//  Set the size of the Output report buffer.  
                            ...
                            //Boolean success;
                            if(selectType=="Control")
                                _myHid.SendOutputReportViaControlTransfer(_hidHandle, outputReportBuffer);
                            else
                            {
                                _myHid.SendOutputReportViaInterruptTransfer2(_deviceData, _hidHandle, outputReportBuffer);
                            }
